...
function SolutionsContent() {
  ...
  const initialId = searchParams.get("id") || solutions[0].id;

  const [selectedSolutionId, setSelectedSolutionId] = useState(initialId);
  const [currentSectorIndex, setCurrentSectorIndex] = useState(0);
  const selectedSolution = solutions.find((s) => s.id === selectedSolutionId);

  // Update selected solution when URL parameter changes
  useEffect(() => {
    const urlId = searchParams.get("id");
    if (urlId && solutions.find((s) => s.id === urlId)) {
      setSelectedSolutionId(urlId);
